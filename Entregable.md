...
| `LogServiceTest.hasAllLogsConcurrent` | El problema era de race condition en la estructura de los logs, se podria llegar a leer o escribir al mismo tiempo y que una de las operaciones no se realice. O se lean valores no actualizados.                                                           | Sincronizamos dentro de addLog() el codigo que hace lectura y escritura de el mapa de logs. Aunque matamos toda la concurrencia... |
...
| `CurrentUserTest.testCurrentConcurrent` | El problema está en getCurrent(), como no esta sincronizado, los hilos comprueban a la vez que current es null, y crean dos usuarios distintos. Es un caso de race condition.                                                                               | Sincronizar el metodo getCurrent().                                                                                                |
...
| `UserServiceTest.testAddNullUser` | El error era que no se hacia ningún control en el metodo addUser() sobre si el User que recibia era null o no.                                                                                                                                              | Agregamos control. Si el User que me llega es null, se lanza una excepcion.                                                        |
| `UserServiceTest.testLoginUser` | En el metodo login() se hace la asignación "lastId = lastId++;". Es incorrecta ya que lastId++ lee y devuelve el valor actual antes de incrementarlo.                                                                                                       | Cambiamos la linea por "lastId++".                                                                                                 |
| `UserServiceTest.testAddUsersAndClear` | El error estaba en que en el metodo clearUsers() solo se limpiaba el repositorio de User. El LogService seguía teniendo referencias a los objetos User, por lo tanto el GC no lo reconocia como basura.                                                     | Agregamos en clearUsers() la linea "logSvc.clear()".                                                                               |
